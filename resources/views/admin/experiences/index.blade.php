            ...
            <form method="POST" action="{{ route('admin.experiences.bulk-destroy') }}">
                ...
                <div class="px-6 py-3 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
                    ...
                    <button type="submit" id="deleteSelectedBtn"
                        ...
                        disabled
                        onclick="return confirm('Apakah Anda yakin ingin menghapus experience yang dipilih?');">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                            <path fill="currentColor"
                                d="M7 21q-.825 0-1.412-.587T5 19V6H4V4h5V3h6v1h5v2h-1v13q0 .825-.587 1.413T17 21zM17 6H7v13h10zM9 17h2V8H9zm4 0h2V8h-2zM7 6v13z" />
                        </svg>
                        Hapus <span id="selectedCount">0</span> Pengalaman
                    </button>
                </div>
                            ...
                            @forelse ($experiences as $item)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <input type="checkbox" name="experience_ids[]" value="{{ $item->id }}"
                                            class="experience-checkbox w-4 h-4 text-blue-600 rounded focus:ring-blue-500">
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        {{ $experiences->firstItem() + $loop->index }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if ($item->image)
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex gap-2">
                                            {{-- <a href="{{ route('admin.experiences.show', $item->id) }}"
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function updateSelectedCount() {
                    const checkedBoxes = document.querySelectorAll('.experience-checkbox:checked');
                ...
                experienceCheckboxes.forEach(checkbox => {
                    checkbox.addEventListener('change', function() {
                        const allChecked = Array.from(experienceCheckboxes).every(cb => cb.checked);
                        const someChecked = Array.from(experienceCheckboxes).some(cb => cb.checked);
